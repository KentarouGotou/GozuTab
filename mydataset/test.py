         ...
         and event["instrument_prefix"] == new_event["instrument_prefix"]:
            ...
            if event["type"]=="note":
                ...
                if(new_event["type"]=="rest"):
                    ...
                    if verbose:
                        print("I was trying to insert a rest. Ignore my rest because there's already a note.")
                    if verbose:
                ...
                if(new_event["type"]=="note"):
                    ...
                    if(event["instrument_prefix"]=="drums"): 
                        ...
                        if(event["fret"]==new_event["fret"]):
                            ...
                            if verbose:
                                print("This drum note is already being played. Ignore my note.")
                            if verbose:
                                print(event,new_event)
                            return False
                    else:
                        ...
                        if(new_event["string"]==event["string"]):
                            ...
                            if verbose:
                                print("There's already a note on this string. Ignore my note.")
                            if verbose:
                                print(event,new_event)
                            return False
                        else:
                ...
                if(new_event["type"]=="note"):
                    ...
                    # Remove the rest. 
                    if verbose:
                        print(" I want to insert a note here. Remove the rest") ####
                    ...
                    # Do nothing. 
                    if verbose:
                        print("I wanted to insert a rest, but there's already a rest here. ")
                    if verbose:
                        print(event,new_event)
                    return False
        # okay now handle cases where a note was already playing on the same instrument
         ...
         and event["instrument_prefix"] == new_event["instrument_prefix"]:
            ...
            if(new_event["type"]=="rest"):
                ...
                # Ignore my new rest.
                if verbose:
                    print("I was trying to insert a rest. There's already a note playing though.")
                return False
...
def guitarpro2json(song, artist, verbose=False):
    ...
    if verbose:
        print("Downtuning scheme of guitar/bass/pads/leads tracks:", downtunages)
    ...
    tempo_token = roundtempo(song.tempo)
    
    head_tokens = {
        "artist": artist,
        "downtune": pitch_shift,
    ...
    for m, _ in enumerate(song.tracks[0].measures):
        ...
        for t, track in enumerate(song.tracks):
            ...
            if not instrument_prefix in head_tokens["instruments"]:
                head_tokens["instruments"].append(instrument_prefix)
            ...
            for v, voice in enumerate(measure.voices):
                ...
                for b, beat in enumerate(voice.beats):
                    ...
                    if(beat.status.name=="empty"): 
                        ...
                        for n, note in enumerate(beat.notes):
                            ...
                            if(test):                            
                                ...
                                if verbose:
                                    print("Note insertion: Oops theres already a note here", m, beat_start, instrument_prefix)
                        ...
                        if(test):
                            ...
                            if verbose:
                                print("Rest insertion: Oops theres already a note here", m, beat_start, instrument_prefix)
                    ...
                    if(len(beatfx)):
                        ...
                        if(test):
                            if verbose:
                                print("Oops theres already a beatfx here", m, beat_start, instrument_prefix)
                        else:
    ...
    for event in events_all:
        ...
        if event["type"] == "measure":
            ...
            if len(event["tokens"]) > 1:
                for measure_token in event["tokens"]:
                    if not measure_token == "new_measure":
                        this_measure["measure_tokens"].append(measure_token)
        ...
        #音符か休符の時
        elif event["type"] == "note" or event["type"] == "rest":
            instrument = event["instrument_prefix"]
            
            if event["type"] == "note":
                token = ":".join([instrument, event["type"], "s" + str(event["string"]), "f" + str(event["fret"])])
            else:
                token = ":".join([instrument, event["type"]])
            current_note = {"token": token}
            current_effect = None 
            
            # Experimental: If there were orphaned nfx, attach them now
            if "effects" in event:
                current_note["nfx"] = event["effects"]
            ...
            if not event["start"] in this_measure["trackbeats"][instrument]:
                # first time this instrument appeared in this measure at this clocktime
                # that means it's a new beat
                current_beat = {"bfx": [], "notes": [], "duration": []}
                this_measure["trackbeats"][instrument][event["start"]] = current_beat
            ...
            this_measure["trackbeats"][instrument][event["start"]]["notes"].append(current_note)
            # add the duration to the beat
            this_measure["trackbeats"][instrument][event["start"]]["duration"] = event["duration"]
        # beat effectの場合
        elif event["type"] == "beatfx":
            instrument = event["instrument_prefix"]
            token = event["effects"][0]
            current_effect = {"token": token, "params": []}
            # add the note to the beat 
            this_measure["trackbeats"][instrument][event["start"]]["bfx"].append(current_effect)
        else:
            all_token.append(this_measure)
    
    all_token = [head_tokens] + all_token[1:]
    return all_token
...
def tokens2guitarpro(all_tokens, verbose=False):
    ...
    artist_token = head["artist"]
    initial_tempo = head["tempo"]
    pitch_shift = head["downtune"]
    instruments = head["instruments"]
    if verbose:
        print(artist_token, initial_tempo, pitch_shift)
    ...
    for instrument in instrument_stringinfo:
        ...
        if(instrument =="bass"):
            ...
            for token in body:
                t = token
                if t["type"]=="note" and t["instrument_prefix"]==instrument:
                    string = t["string"] # ex "s4" 
                    fret = t["fret"] # ex "f5"
                    if fret == -1 or fret == -2:
                        if string == 6 or string == 7:
                            drop_tuning = True
    ...
    for i,token in enumerate(body):
        ...
        if token["tokens"][0] in ["end", "new_measure"] and len(this_measure):
        ...
        if token["tokens"][0]=="new_measure":
            ...
            if(t["type"]=="measure"):
            ...
            elif t["type"]=="note" or t["type"]=="rest":
                ...
                instrument = t["instrument_prefix"]
...
def conversion(input_file, output_file, direction, artist_token="unknown"):
...
Instrument changes are not supported. Banjos are not supported. 
    ...
    if sys.argv[1] == "encode":
        if os.path.isdir(sys.argv[2]):
            assert os.path.isdir(sys.argv[3]), "Error: When you write a folder as input, the output must also be a folder."
            for file in os.listdir(sys.argv[2]):
                if os.path.splitext(file)[-1] in [".gp3", ".gp4", ".gp5"]:
                    input_file = sys.argv[2] + "/" + file
                    output_file = sys.argv[3] + "/" + os.path.splitext(file)[0] + ".json"
                    direction = sys.argv[1]
                    try:
                        artist_token = sys.argv[4]
                    except:
                        artist_token = "unknown" # default
                    conversion(input_file, output_file, direction, artist_token)
        elif os.path.splitext(sys.argv[2])[-1] in [".gp3", ".gp4", ".gp5"]:
            assert os.path.splitext(sys.argv[3])[-1] == ".json", "Error: When you write a GuitarPro file as input, the output must also be a Json file."
            ...
            print("Error: Inputs must be in a folder or GuitarPro file.")
        ...
        if os.path.isdir(sys.argv[2]):
            ...
            for file in os.listdir(sys.argv[2]):
                if os.path.splitext(file)[-1] == ".json":
                    ...
                    conversion(input_file, output_file, direction, artist_token)
        #a Json file
        elif os.path.splitext(sys.argv[2])[-1] == ".json":
            assert os.path.splitext(sys.argv[3])[-1] in [".gp3", ".gp4", ".gp5"], "Error: When you write a Json file as input, the output must also be a GuitarPro file."
            input_file = sys.argv[2]
            output_file = sys.argv[3]
            direction = sys.argv[1]
            try:
            ...
            print("Error: Inputs must be in a folder or Jason file.")
